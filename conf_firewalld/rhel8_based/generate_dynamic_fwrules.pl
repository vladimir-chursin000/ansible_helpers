...
sub generate_shell_script_for_recreate_firewall_zones {
    ...
    my @recreate_std_fwzones_commands_l=();
    my %wr_hash_l=();
	#key0=inv-host, key1=wr_type (standard, custom, etc), value=array of strings
    my $return_str_l='OK';
    
    # fill array with strings for scripts at all hosts
    @begin_script_arr_l=('#!/usr/bin/bash',' ','###GENERATED BY ansible_helpers/conf_firewalld (https://github.com/vladimir-chursin000/ansible_helpers)','###DO NOT CHANGE!',' ');
    ###
    
    # fill array with strings/commands for recreate std fwzones at all hosts
    @recreate_std_fwzones_commands_l=();
    ###
