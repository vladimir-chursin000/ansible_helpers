...
our %inv_hosts_hash1_g=(); #key0=inv_host, key1=now/fin (generated by this script)/for_upd/for_del value=array of ifcfg-files names
...
if ( $gen_playbooks_next_g==1 ) { # if need generate of dynamic playbooks for ifcfg upd/del
    ...
    #%inv_hosts_hash1_g=(); #key0=inv_host, key1=now/fin (generated by this script)/for_upd/for_del value=array of ifcfg-files names
    
    while ( ($hkey0_g,$hval0_g)=each %inv_hosts_hash0_g ) {
	#hkey0_g=inv_host
	opendir(DIR_FIN,$dyn_ifcfg_common_dir_g.'/'.$hkey0_g.'/fin');
	@{$inv_hosts_hash1_g{$hkey0_g}{'fin'}}=grep {!(/^\./)} readdir(DIR_FIN);
	closedir(DIR_FIN);
	
	opendir(DIR_NOW,$ifcfg_backup_from_remote_dir_g.'/'.$hkey0_g);
	@{$inv_hosts_hash1_g{$hkey0_g}{'now'}}=grep {!(/^\.|^ifcfg\-lo$/)} readdir(DIR_NOW);
	closedir(DIR_NOW);
    }
    ($hkey0_g,$hval0_g)=(undef,undef);
    print Dumper(\%inv_hosts_hash1_g);
}
