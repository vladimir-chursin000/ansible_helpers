sub read_inventory_file {
    ...
    while ( <INVDATA> ) {
        ...
        if ( length($line_l)>0 && $line_l!~/^\#/ ) {
            if ( $line_l=~/^\[firewall_conf_dedic_app_v1\]/ && $start_read_hosts_flag_l==0 ) { $start_read_hosts_flag_l=1; }
            elsif ( $start_read_hosts_flag_l==1 && $line_l=~/^\[firewall_conf_dedic_app_v1\:vars\]/ ) {
