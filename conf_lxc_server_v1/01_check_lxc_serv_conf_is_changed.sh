...
INV_LIMIT_str='no';
PLAYBOOK_str='check_lxc_serv_conf_is_changed_playbook.yml';
...
do
    if [[ "$TMP_VAR_str" =~ ^"limit=" ]]; then
        # possible argv values: host_name, "host_name1,host_name2,..." (host names from inventory = "lxc_server_hosts"),
...
$SELF_DIR_str/main.sh "$INV_LIMIT_str" "$PLAYBOOK_str" "$LOG_DIR_str";
